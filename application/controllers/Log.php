	...
	function in()
	{
		...
		else
		{
			...
			if ($this->form_validation->run() == TRUE)
			{
					...
					foreach ($check_username as $check_hash)
					{
						...
						if ($check_password == TRUE)
						{
							...
							foreach ($looping_user as $user)
							{
								...
								{
									$data = array(
										'errors' => '<p>Your account status is not yet active, please check your email address.</p>'
										);
						...
						{
							$data = array(
								'errors' => '<p>The password you entered is incorrect.</p>'
								);
							$this->_render('auth/log_in', $data);
						}						
					}
				...
				{
					$data = array(
						'errors' => '<p>The username does not exist in the database.</p>'
						);
					$this->_render('auth/log_in', $data);
				}
			}
